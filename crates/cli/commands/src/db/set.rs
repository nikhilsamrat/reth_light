...
impl Command {
    ...
    pub fn execute<N: NodeTypesWithDB>(self, provider_factory: ProviderFactory<N>) -> eyre::Result<()> {
        match self.subcommand {
            Subcommand::Mdbx { table, key, subkey, value } => {
                table.view(&SetValueViewer { db: provider_factory.db_ref(), key, subkey, value })?
            }
            Subcommand::StaticFile { segment, key, value } => {
...
struct SetValueViewer<'a, DB: Database> {
    db: &'a DB,
    key: String,
    subkey: Option<String>,
    value: String,
...
impl<DB: Database> TableViewer<()> for SetValueViewer<'_, DB> {
    type Error = eyre::Report;

    fn view<T: Table>(&self) -> Result<(), Self::Error> {
        let key = table_key::<T>(&self.key)?;
        let value = serde_json::from_str::<T::Value>(&self.value)?;

        let res = self.db.update(|tx|{
            tx.put::<T>(key, value)?;
            Ok(())
        })?;
